...
Base package has a simple summary plotting feature for regression objects, but it is not exactly nice looking.
```{r baseplot}
plot( lm("Sepal.Length ~ Species + Petal.Length", data=iris), which=1 )
```


I thought it was appropriate to write a summary plotter that looked nicer, and used Hadley Wickham's ```ggplot2``` package.

The function contains many, many parameters. Default settings return plots like the above ```plot``` call, and then some.

1. ```model``` takes a object of class lm. 
2. ```toplot``` can specify plots return labels for points
3. ```which``` which of the 7 summary plots to use
4. ```mfrow``` is x-y list of dimensions of grid to print out.
5. ```smooth``` is method for geom_smooth
6. ```stderr``` shows standard error geom_smooth appear
7. ```printPlot``` employs ```plot``` style paused viewing
8. ```grobOut``` return graphical object


The full content of the function:
```{r funcContent, eval=FALSE}
...
Example invocation:
```{r funcDisplay, eval=F}
bettrPlot( lm("Sepal.Length ~ Species + Petal.Length", data=iris) )
```
```{r funcCall, echo=F, message=F}
bettrPlot  <- function(model, toplot="points", which=c(1:3, 5), mfrow=c(1,1), smooth="auto", rView=T, stderr=TRUE,
											 returnwhat= "plots", printPlot=T, grobOut=T, ...) {
...
suppressWarnings(
	g <- bettrPlot( lm("Sepal.Length ~ Species + Petal.Length", data=iris), grobOut = T, printPlot = F )
	)
ggsave(file="plot.png", plot=g, dpi=800)
```
![Plot Image](plot.png)
